function checkLatestEntries() {
  ...
  const pertussisStartDate =
    document.getElementById("startDatePertussis").value;
...
function displayResults(data) {
  ...
  covidMessage.textContent = data.covidEntries
    ? "Recent Covid Vaccinations found!"
    : "No recent Covid Vaccinations found.";
  document.querySelector("section.box:nth-child(1)").appendChild(covidMessage);

  const pertussisMessage = document.createElement("p");
  pertussisMessage.textContent = data.pertussisEntries
    ? "Recent Pertussis Vaccinations found!"
    : "No recent Pertussis Vaccinations found.";
  document
    .querySelector("section.box:nth-child(2)")
    .appendChild(pertussisMessage);
}
